...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        ...
        if len(que) > 0:
            ...
            stats += "🎵 ꜱᴏɴɢ ᴘʟᴀʏᴇᴅ: `{}`\n".format(len(que))
            stats += "💡 ɴᴏᴡ ᴘʟᴀʏɪɴɢ: **{}**\n".format(queue[0][0])
            stats += "🎧 VENOM ᴜꜱᴇʀ ʙʏ: {}".format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def ytplay(_, message: Message):
    ...
    for i in message.command[1:]:
        query += " " + str(i)
    print(query)
    await lel.edit("🔄 **ᴄᴏɴɴᴇᴄᴛɪɴɢ ᴛᴏ VENOM ꜱᴇʀᴠᴇʀ...**")
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                ...
                InlineKeyboardButton("ꜱᴜᴘᴘᴏʀᴛ ⚡", url=f"https://t.me/VENM_xd"),
            ],
        ]
    )
    message.from_user.first_name
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            ...
            + f"🎧 **VENOM ᴜꜱᴇʀ ʙʏ:** {message.from_user.mention}",
            reply_markup=keyboard,
        )
        os.remove("final.png")
